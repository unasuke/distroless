"Macros for generating ldconfig cache for ruby images"

load("@rules_oci//oci:defs.bzl", "oci_load")

def ruby_ldconfig(architectures, distro):
    """Generates ldconfig cache targets for ruby images.
    ...
    for arch in architectures:
        oci_load(
            name = "load_ruby_root_{}_{}".format(arch, distro),
            image = ":ruby_root_{}_{}".format(arch, distro),
            repo_tags = ["bazel/ruby:ruby_root_{}_{}".format(arch, distro)],
        )
    ...
    for arch in architectures:
        native.genrule(
            ...
            cmd = "$(location :load_ruby_root_{arch}_{distro}) && touch $@".format(
                arch = arch,
                distro = distro,
            ),
            tags = [
            ...
            tools = [":load_ruby_root_{}_{}".format(arch, distro)],
        )
