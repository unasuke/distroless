...
for arch in "$@"; do
  ...
  CID=$(docker create --platform "linux/${arch}" --entrypoint /sbin/ldconfig "bazel/ruby:ruby_root_${arch}_debian13")
  docker start -a "${CID}"
  ...
  if [[ "${MODE}" == "update" ]]; then
    ...
    if ! cmp -s "${TEMP_DIR}/ld.so.cache.${arch}" "${ROOT}/ld.so.cache.${arch}"; then
      ...
      echo "Run 'bazel run //ruby:update_ldconfig' to regenerate." >&2
      exit 1
    fi
    rm -rf "${TEMP_DIR}"
    TEMP_DIR=""
